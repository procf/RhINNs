...
class PINNSolver():
    ...
    def loss_fn(self, X_data, X_f0, y_data):
        R1, R2 = self.get_r()
        Loss_eq = 1.0*(tf.reduce_mean(tf.square(R1)) + tf.reduce_mean(tf.square(R2)))
        y_pred = self.model(X_data)
        y_init = self.model(X_f0)
        Loss_init = self.wic*(tf.reduce_mean(tf.square(tf.gather(y_init, [0], axis=1) - 0.0))\
         + tf.reduce_mean(tf.square(tf.gather(y_init, [1], axis=1) - 1.)))
        Loss_data = self.wdata*(tf.reduce_mean(tf.square(y_data - tf.gather(y_pred, [0], axis=1))))
...
def read_data():
    df = pd.read_excel('StartUp.xlsx', sheet_name=None)
